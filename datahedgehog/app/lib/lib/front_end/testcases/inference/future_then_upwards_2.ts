/** Library asset:datahedgehog_monitor/lib/lib/front_end/testcases/inference/future_then_upwards_2.dart */
...
export var main : () => void = () : void =>  {
    ...
    let f2 : MyFuture<number> = f;
    let f3 : MyFuture<number> = foo().then((_ : any) =>  {
        return 2.3;
    }) as MyFuture<double>;
};
export var foo : () => MyFuture<any> = () : MyFuture<any> =>  {
    return new MyFuture.value(1);
};
