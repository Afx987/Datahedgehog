...
var CommonUsageSorter = /** @class */ (function () {
    function CommonUsageSorter(selectorRelevance) {
    }
    CommonUsageSorter.prototype.CommonUsageSorter = function (selectorRelevance) {
        selectorRelevance = selectorRelevance || properties.defaultSelectorRelevance;
        this.selectorRelevance = selectorRelevance;
    };
    CommonUsageSorter.prototype.sort = function (request, suggestions) {
        this._update(request, suggestions);
        return new async.Future.value();
    };
    CommonUsageSorter.prototype._getCompletionTarget = function (request) {
        return new bare.createInstance(any, null, request.result.unit, request.offset);
    };
    CommonUsageSorter.prototype._update = function (request, suggestions) {
        var target = this._getCompletionTarget(request);
        if (target != null) {
            var visitor = new _BestTypeVisitor(target.entity);
            var type = target.containingNode.accept(visitor);
            if (type != null) {
                var typeElem = type.element;
                if (typeElem != null) {
    ...
    CommonUsageSorter.prototype._updateInvocationRelevance = function (type, libElem, suggestions) {
        var typeName = type.name;
        var selectors = this.selectorRelevance.get(libElem.name + "." + typeName);
        if (selectors != null) {
            for (var _i = 0, suggestions_1 = suggestions; _i < suggestions_1.length; _i++) {
                ...
                if (element != null && (utils_1.op(utils_1.Op.EQUALS, element.kind, lib3.ElementKind.CONSTRUCTOR) || utils_1.op(utils_1.Op.EQUALS, element.kind, lib3.ElementKind.FIELD) || utils_1.op(utils_1.Op.EQUALS, element.kind, lib3.ElementKind.GETTER) || utils_1.op(utils_1.Op.EQUALS, element.kind, lib3.ElementKind.METHOD) || utils_1.op(utils_1.Op.EQUALS, element.kind, lib3.ElementKind.SETTER)) && utils_1.op(utils_1.Op.EQUALS, suggestion.kind, CompletionSuggestionKind.INVOCATION) && utils_1.op(utils_1.Op.EQUALS, suggestion.declaringType, typeName)) {
                    var index = selectors.indexOf(suggestion.completion);
                    if (index != -1) {
                        suggestion.relevance = utils_1.op(utils_1.Op.MINUS, DART_RELEVANCE_COMMON_USAGE, index);
                    }
                }
            }
        }
    ...
    ], CommonUsageSorter.prototype, "CommonUsageSorter", null);
    __decorate([
    ...
    ], CommonUsageSorter.prototype, "sort", null);
    CommonUsageSorter = __decorate([
        utils_1.DartClass,
        utils_1.Implements(any)
    ], CommonUsageSorter);
    return CommonUsageSorter;
}());
exports.CommonUsageSorter = CommonUsageSorter;
var _BestTypeVisitor = /** @class */ (function (_super) {
    __extends(_BestTypeVisitor, _super);
    function _BestTypeVisitor(entity) {
        var _this = this;
        return _this;
    }
    _BestTypeVisitor.prototype._BestTypeVisitor = function (entity) {
        this.entity = entity;
    };
    _BestTypeVisitor.prototype.visitConstructorName = function (node) {
        return node.period != null && utils_1.op(utils_1.Op.EQUALS, node.name, this.entity) ? (function (t) { return (!!t) ? t.type : null; })(node.type) : null;
    };
    _BestTypeVisitor.prototype.visitNode = function (node) {
        return null;
    };
    _BestTypeVisitor.prototype.visitPrefixedIdentifier = function (node) {
        return utils_1.op(utils_1.Op.EQUALS, node.identifier, this.entity) ? (function (t) { return (!!t) ? t.bestType : null; })(node.prefix) : null;
    };
    _BestTypeVisitor.prototype.visitPropertyAccess = function (node) {
        return utils_1.op(utils_1.Op.EQUALS, node.propertyName, this.entity) ? (function (t) { return (!!t) ? t.bestType : null; })(node.realTarget) : null;
    };
    __decorate([
        utils_1.defaultConstructor
    ], _BestTypeVisitor.prototype, "_BestTypeVisitor", null);
    _BestTypeVisitor = __decorate([
        utils_1.DartClass
    ], _BestTypeVisitor);
    return _BestTypeVisitor;
}(any));
exports._BestTypeVisitor = _BestTypeVisitor;
var properties = /** @class */ (function () {
    function properties() {
    }
    Object.defineProperty(properties, "defaultSelectorRelevance", {
        get: function () {
...
//# sourceMappingURL=common_usage_sorter.js.map